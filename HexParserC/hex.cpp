...
{
	char c[3];
	printf("\nTesting single character to uint8_t\n");
	for (uint8_t i = 0; i < 16; ++i)
	{
		itoa(i, c, 16);
		printf("0x%s: %d\n", c, hexCharToUint8(c[0]));
	}
}
...
{
	char c[4];
	printf("\nTesting 2 character to uint8_t\n");
	for (uint16_t i = 16; i < 256; ++i)
	{
		itoa(i, c, 16);
		printf("0x%s: %d\n", c, hexCharsToUint8(c[0], c[1]));
	}
}
...
{
	char c[6];
	printf("\nTesting 4 character to uint16_t\n");
	for (uint32_t i = 0x1000; i < 0xFFFF; i+=537)
	{
		itoa(i, c, 16);
		printf("0x%s: %d\n", c, hexCharsToUint16(c[0], c[1], c[2], c[3]));
	}
}
...
uint32_t parseHexFileEntry(char *data, uint32_t offset, HexFileEntry_t *hexEntry)
{
	...
	//Get record type
	hexEntry->recordType = (RecordType)hexCharsToUint8(data[offset + 7], data[offset + 8]);
